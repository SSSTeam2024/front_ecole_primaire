...
import { useUpdateInscriptionNotesMutation } from "features/inscriptions/inscriptionSlice";
import Swal from "sweetalert2";

const UpdateInscription = () => {
  document.title = " Modifier Inscription | Sousse Leaders School";
  ...
  const handleInscriptionNote = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInscriptionNote(e.target.value);
  };
  ...
  const notifySuccess = () => {
    Swal.fire({
      position: "center",
      icon: "success",
      title: "Notes ajoutés au inscription avec succès",
      showConfirmButton: false,
      timer: 2500,
    });
  };

  const [updateNotes] = useUpdateInscriptionNotesMutation();
  ...
  const handleUpdateGroupe = (notes: any) => {
    if (details) {
      updateNotes({ _id: details._id, notes })
        .then(() => {
          notifySuccess();
        })
        .then(() => {
      ...
      <div className="page">
        <Container fluid>
          ...
          {details.notes && details.notes !== "" ? (
            ...
            <Card>
              <Card.Header>
                <span className="fw-medium fs-18">Notes</span>
              </Card.Header>
              <Card.Body>
                <Row>
                  <small className="mb-1">
                    Ecrire quelques notes sur l'étudiant et ses notes
                  </small>
                  <Col lg={8}>
                    <Form.Control
                      type="text"
                      id="inscriptionNote"
                      name="inscriptionNote"
                      onChange={handleInscriptionNote}
                  ...
                  <Col>
                    <Button
                      ...
                      onClick={() => handleUpdateGroupe(inscriptionNote)}
                    >
                      Ajouter
                    </Button>
                  </Col>
